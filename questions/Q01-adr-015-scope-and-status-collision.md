...
## Shingoからの回答 (2026-05-07)

### Q1-A: 実装の分割方針
→ Option A（段階分割）で進める。本PRでDB基盤（テーブル追加）のみ実装し、
  AI連携・ダッシュボード・ステータス移行は続くPRで対応する。

### Q1-B: prospect_rank と ステータス設計
→ prospect_rank（A/B+/B/B-/仮C/確定C）は残す・廃止しない。
  ADR-015で自動化される「返信速度」「想定規模」をインプットとして活用する。
  「温度感」「顧客タイプ」は引き続き営業担当が手動入力する。
  既存LeadStatusは置き換えではなく拡張（既存データの移行スクリプトは不要）。

### Q1-C: ダッシュボード
→ 既存のKPIグラフ画面は残す。
  OVERDUE / TODAY / TOMORROW / UPCOMING は新しいセクションとして追加する。
  next_action_date カラムは leads テーブルに追加する。

### Q1-D: LLM接続
→ 技術判断はパートナーに委任。
  ASTRO_SERVER_B_URL は環境変数で渡す（本番.envに追記）。
  Claude Haiku 4.5 は既存の ANTHROPIC_API_KEY を使用。
  コスト上限の強制機構は不要、運用監視のみで足りる。

### Q1-E: SNS dedup テーブル拡張
→ ADR-015のスコープ内で対応する。
  customer_channels テーブル（全チャンネル対応）を新規作成する。
  Telegram連携は今回スコープ外。
