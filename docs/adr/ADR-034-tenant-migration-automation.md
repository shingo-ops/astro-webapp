...
| 関連 | ADR-028 (撮影テナント分離), ADR-036 (テナントスキーマ整合性保証), ADR-072 (tenant schema prefix 統一) |
...
本 ADR の `deploy.yml` 全テナント適用ループが稼働するまでの間、新規 migration 追加時は SQL に加えて全既存テナントへの手動適用ステップを PR body に明記する。
