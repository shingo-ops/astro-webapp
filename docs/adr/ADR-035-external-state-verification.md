...
| 作成日 | 2026-05-15 |
| 起案 | ひとし（森本） |
| 関連 ADR | ADR-024（Meta 連携構造的修正）/ ADR-025（Meta 運用 hardening）/ ADR-026（IG message_id TEXT 化）/ ADR-029（self-hosted runner 2 台体制）/ ADR-034（テナント migration 自動化）/ ADR-036（テナントスキーマ整合性）/ ADR-038（QA Smoke Suite）/ ADR-039（Generator codebase reconnaissance） |
...
mkdir -p "$OUT"
# Graph API
curl -s "https://graph.facebook.com/v18.0/${META_APP_ID}?access_token=${META_APP_TOKEN}" \
  | jq 'del(.access_token, .secret) | .secret_sha256 = (.secret_present // false)' \
  > "$OUT/meta_app.json"
curl -s "https://graph.facebook.com/v18.0/${META_APP_ID}/subscriptions?access_token=${META_APP_TOKEN}" \
  > "$OUT/meta_subscriptions.json"
curl -s "https://graph.facebook.com/v18.0/${META_PAGE_ID}/subscribed_apps?access_token=${META_PAGE_TOKEN}" \
...
**secret scrub ルール**: snapshot json には raw secret を **絶対に書かない**。常に `sha256:<8byte>` prefix または `*_present: true/false` のいずれかに置換。Reviewer は merge 前に各 snapshot.sh の scrub ロジックを目視確認。
...
| `METADATA_FERNET_KEY` | env | (同上) | 9 files | backend/app/services/encryption.py | Keep | 実在 |
...
Total referents: 18  /  0-hit replaced: 0（全て新規 Add）/  Halted: 0

`CLOUDFLARE_API_TOKEN` は repo に grep hit なし → しんごさん側で実存確認 + 新規追加が必要（Cloudflare DNS で salesanchor.jp / jarvis-claude.uk 等を管理しているが、API token を未保存の可能性）。本 ADR の実装 PR では `scripts/snapshot/cloudflare.sh` を最小限の構造で配置し、token 設定待ち期間は `--dry-run` のみ動作するよう実装する。
