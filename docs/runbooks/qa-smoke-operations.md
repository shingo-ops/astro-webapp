...
- **2 層排他構造**:
  - **L1 (workflow 単位)**: `.github/workflows/qa-smoke.yml` の `concurrency.group: qa-smoke-tenant-006` で GitHub Actions 上の並列 workflow run を直列化
  - **L2 (process 単位)**: `flock /tmp/qa-tenant-006.lock` で同一 runner 上の reset / 撮影 / 別ジョブを最大 10 分待機で直列化
...
- 対処:
  ...
  2. `DRY_RUN=1 bash scripts/qa/cleanup-smoke-data.sh` で件数確認
...
- 対処: `DRY_RUN=1 bash scripts/qa/cleanup-smoke-data.sh` で件数を見て、想定外の残骸が判明したら
