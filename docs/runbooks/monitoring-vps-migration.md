...
|----------|---------|:---:|------|-----------|------|
| M1 | Phase 1: VPS契約・ファイアウォール・Docker | 未着手 | PO待ち | さくらVPS未契約 | 2026-05-29 |
| M2 | Phase 2: 監視スタック移動（設定ファイル転送・コンテナ起動） | 未着手 | - | M1完了待ち | 2026-05-29 |
| M3 | Phase 3: VPS間通信設定（Prometheus scrape・promtail） | 未着手 | - | M2完了待ち | 2026-05-29 |
| M4 | Phase 4: Grafana プロキシ変更 | 未着手 | - | M3完了待ち | 2026-05-29 |
| M5 | Phase 5: 動作確認チェックリスト | 未着手 | - | M4完了待ち | 2026-05-29 |
| M6 | Phase 6: exporter 専用 docker-compose 分離 | 未着手 | - | M5完了待ち | 2026-05-29 |
| M7 | Phase 7: アプリVPS旧監視コンテナ撤去 | 未着手 | - | M6完了待ち | 2026-05-29 |
| M8 | 旧ボリューム削除（PO確認後・1週間運用後） | 未着手 | PO確認必須 | M7完了+1週間後 | 2026-05-29 |

**状態の値**: `未着手` / `進行中` / `完了` / `ブロック`

> 管理室VPSのIPが確定したら、このファイル内の `MGMT_PUBLIC_IP` プレースホルダーを実際のIPに置換すること。
...
3. パブリックIPを記録: `________` (以下 `MGMT_PUBLIC_IP` と表記)
...
| 許可ポート | プロトコル | 送信元 | 用途 |
|-----------|----------|--------|------|
| 9100 | TCP | `MGMT_PUBLIC_IP` のみ | node-exporter scrape |
| 9187 | TCP | `MGMT_PUBLIC_IP` のみ | postgres-exporter scrape |
| 9113 | TCP | `MGMT_PUBLIC_IP` のみ | nginx-exporter scrape |
| 9121 | TCP | `MGMT_PUBLIC_IP` のみ | redis-exporter scrape |
...
scp <USER>@49.212.137.46:/path/to/salesanchor/monitoring/prometheus/prometheus.yml \
    <USER>@MGMT_PUBLIC_IP:/opt/salesanchor-monitoring/monitoring/prometheus/
...
scp -r <USER>@49.212.137.46:/path/to/salesanchor/monitoring/grafana/provisioning/* \
    <USER>@MGMT_PUBLIC_IP:/opt/salesanchor-monitoring/monitoring/grafana/provisioning/

# Loki 設定
scp <USER>@49.212.137.46:/path/to/salesanchor/monitoring/loki/loki-config.yaml \
    <USER>@MGMT_PUBLIC_IP:/opt/salesanchor-monitoring/monitoring/loki/

# Loki ルール
scp -r <USER>@49.212.137.46:/path/to/salesanchor/monitoring/loki/rules/* \
    <USER>@MGMT_PUBLIC_IP:/opt/salesanchor-monitoring/monitoring/loki/rules/

# gha-exporter
scp -r <USER>@49.212.137.46:/path/to/salesanchor/monitoring/gha-exporter/* \
    <USER>@MGMT_PUBLIC_IP:/opt/salesanchor-monitoring/monitoring/gha-exporter/
```
...
promtail は `PROMTAIL_LOKI_URL=http://MGMT_PUBLIC_IP:3100/loki/api/v1/push` を環境変数として渡し、
...
```yaml
clients:
  - url: http://MGMT_PUBLIC_IP:3100/loki/api/v1/push
```
...
location /grafana/ {
    proxy_pass http://MGMT_PUBLIC_IP:3000/grafana/;  # 管理室VPSに向ける
...
1. [PO] Cloudflare DNS で `grafana.salesanchor.jp` の A レコードを `MGMT_PUBLIC_IP` に設定（プロキシ ON）
...
```bash
# 管理室VPSの Grafana を停止
ssh <USER>@MGMT_PUBLIC_IP "cd /opt/salesanchor-monitoring && docker compose -f docker-compose.monitoring.yml stop grafana"
