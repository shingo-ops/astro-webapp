...
| M7 | Phase 7: 管理室VPSへ監視スタックをデプロイ・動作確認・旧コンテナ撤去 | 完了 | Claude Code | PR #1148 #1150 | 2026-05-29 |
| M8 | 旧ボリューム削除（PO確認後・1週間運用後） | 未着手 | PO確認必須 | 2026-06-05 以降 | 2026-05-29 |
...
さくらVPS コントロールパネルで **パケットフィルタを先に設定** する。
今回の調査では、VPS 内 `ufw` を開けてもアプリVPSから監視VPSへパケットが届かなかったため、**提供元側のパケットフィルタと VPS 内 `ufw` の両方** を揃える必要がある。

**画面上でやること**

1. さくらのVPS コントロールパネルを開く
2. 監視VPS `49.212.160.98` を選ぶ
3. `パケットフィルタ設定` を開く
4. `パケットフィルタを設定` を押す
5. `パケットフィルタ設定を追加する` を押す
6. `カスタム` を選ぶ
7. 以下の 3 件を 1 件ずつ追加する
8. 最後に `設定を保存する` を押す
...
| プロトコル | ポート | 送信元 | 動作 |
|-----------|------|--------|------|
| TCP | 3000 | `49.212.137.46` | 許可 |
| TCP | 3001 | `49.212.137.46` | 許可 |
| TCP | 9090 | `49.212.137.46` | 許可 |

**入力のしかた**

- 1件目: `TCP / 3000 / 49.212.137.46 / 許可`
- 2件目: `TCP / 3001 / 49.212.137.46 / 許可`
- 3件目: `TCP / 9090 / 49.212.137.46 / 許可`

**入れ終わったら**

1. `設定を保存する` を押す
2. 監視VPS の `ufw` にも同じ許可が入っていることを確認する
3. アプリVPS から `3000/3001/9090` の疎通確認をする
...
> 監視VPS の `3000/3001/9090` は、少なくともアプリVPS `49.212.137.46` から届くようにする。`ufw` だけではなく、さくらVPS のパケットフィルタでも同じ許可を入れること。

### ダッシュボードの色の意味と反映確認

Grafana のダッシュボードでは、色を以下の意味で統一する。
...
repo の JSON を直しただけでは、監視VPS の Grafana 画面はまだ変わらない。実機反映は、監視VPS 上の Grafana が provisioning を読み込んだ後に起きる。
...
bash scripts/check-task-state.sh

# 2. 監視VPS 側で Grafana が動いていることを確認
ssh <USER>@49.212.160.98 "cd /opt/salesanchor-monitoring && docker compose -f docker-compose.monitoring.yml ps grafana"

# 3. dashboard JSON の再読み込みを待つ
# dashboards.yml の updateIntervalSeconds は 30 秒
# 変更が急ぐ場合のみ Grafana を再起動する
ssh <USER>@49.212.160.98 "cd /opt/salesanchor-monitoring && docker compose -f docker-compose.monitoring.yml restart grafana"

# 4. ブラウザで新しい文言と色が出ることを確認
# 例: 監視メインの説明文、総合評価の色、部門サマリーの緑/黄/赤
```
