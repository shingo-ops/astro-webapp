...
## 起動方法 (ADR-056 以降 — develop までの完全自動化)

| エージェント / フェーズ | 起動 |
|---|---|
| Generator (初回実装) | claude-pipeline.yml `claude-worker` job (自動) |
| Reviewer | claude-pipeline.yml `reviewer` job (自動) |
| Evaluator | claude-pipeline.yml `evaluator` job (自動) |
| Generator (自動修正) | claude-pipeline.yml `regenerate` job (自動、最大 3 回) |
| **develop merge** | claude-pipeline.yml `automerge` job (自動、両 APPROVED 時) |
| **main merge** | 人間 (しんごさん、手動) — `gh pr merge` で release PR をマージ |

人間が個別に Terminal CC に「reviewer.md でレビューしてくれ」「regenerate して」と指示する必要は **ない**。
PR が立った時点で自動的に Reviewer → Evaluator → (両 APPROVED なら) automerge / (いずれか REQUEST_CHANGES なら) regenerate → 再 Reviewer のループが走る。

### ADR-056 の自動化フロー全体図

```
ADR push (workflow_dispatch)
  ↓
...
reviewer 自動起動
  ...
  │     ↓
  │     ├─ APPROVED → automerge job → develop merged
  │     │                                  ↓
  │     │                              Discord 通知 1 件: 「develop merged」
  │     │
  │     └─ REQUEST_CHANGES → regenerate job 自動起動
  │
  └─ REQUEST_CHANGES → regenerate job 自動起動
        ↓
        ├─ retry ≤ 3 → Generator が修正 push → pull_request:synchronize → reviewer 再起動 (ループ)
        └─ retry > 3 or 同一指摘繰り返し → needs-human-intervention label 付与 + Discord 通知 + 停止
```

### 安全装置

- `pause-automation` label を PR に付けると、automerge / regenerate が即時停止 (緊急停止)
- `needs-human-intervention` label が付いた PR も自動化対象から外れる
- `--delete-branch` は claude-impl/* / hotfix/* のみ (ADR-050 規約) — develop / main は保護
- main merge は引き続き人間判断 (ADR-056 §2-6 / §4 Scope 外)

### Discord 通知の整理 (ADR-056 §2-4)

| イベント | 通知 |
|---|---|
| claude-worker 開始 / 完了 / reviewer 各種 / evaluator 各種 | ❌ なし |
| **develop merge 完了** | ✅ あり (1 ADR で通常 1 件のみ) |
| **リトライ上限到達 (人間介入必要)** | ✅ あり |
| **job 例外 FAILURE** (claude-worker / automerge / regenerate) | ✅ あり |
...
### 4. `~/.claude/agents/reviewer.md` への ADR-051 追記

本文先頭 (`# Role and order in the pipeline` セクションの直前) に以下を追加:

```markdown
# 自動起動 (ADR-051)

この Agent は `claude-pipeline.yml` `reviewer` job によって自動起動されます。Generator が PR を作成した後、人間の手動指示なしに自動的に起動します。

```

### 5. `~/.claude/agents/evaluator.md` への ADR-051 追記

本文先頭 (pipeline 図の直前) に以下を追加:

```markdown
# 自動起動 (ADR-051)

この Agent は `claude-pipeline.yml` `evaluator` job によって自動起動されます。Reviewer が APPROVED した後、人間の手動指示なしに自動的に起動します。
...
# ADR-056: develop までの完全自動化 (新フロー反映確認)
grep -c "auto-merge to develop" ~/.claude/agents/generator.md
# 期待: 1 以上 (ADR-056 §2-8 で要求される grep 項目)
...
# ADR-056: claude-pipeline.yml 側の構造確認 (リポジトリ側で実行)
grep -c "^  regenerate:" .github/workflows/claude-pipeline.yml
# 期待: 1 (regenerate job が定義されている)

grep -c "^  automerge:" .github/workflows/claude-pipeline.yml
# 期待: 1 (automerge job が定義されている)

grep -c "pause-automation" .github/workflows/claude-pipeline.yml
# 期待: 2 以上 (緊急停止ラベルの check と create)

grep -c "needs-human-intervention" .github/workflows/claude-pipeline.yml
# 期待: 2 以上 (上限到達時の label 付与と check)

grep -c "MAX_AUTO_RETRIES" .github/workflows/claude-pipeline.yml
# 期待: 2 以上 (リトライ上限の env と比較)
```

## 適用後の挙動 (要約 — 4 エージェント網羅、ADR-056 適用後)

| 役割 | 動作モード | 入力 | 操作 | 出力 |
|---|---|---|---|---|
| **Planner** | **Plan mode** (write/exec 封じる安全モード) | アイディア、ビジネス要求 | 読み込み (Read/Grep/Glob) + ADR or 仕様書を起案 | ADR or 仕様書 (Generator に渡す) |
| **Generator (初回)** | 通常モード | ADR or 仕様書 (Planner から) | 実装 + `git commit` + `git push` + `gh pr create` + Evaluator skip 宣言 | PR open |
| **Reviewer** | 通常モード | Generator が open した PR | PR コメント `## Reviewer 判定: APPROVED | REQUEST_CHANGES` (ADR-053、`gh pr review --approve` は self-approve 制限で不可) | decision output `APPROVED` / `REQUEST_CHANGES` |
| **Evaluator** | 通常モード (Skip 宣言なら自動 APPROVED 扱い) | Reviewer APPROVE 済 PR | Playwright で UI/UX 評価 → PR コメント `## Evaluator 判定: ...` | decision output `APPROVED` / `REQUEST_CHANGES` |
| **Generator (regenerate)** | 通常モード | reviewer / evaluator の REQUEST_CHANGES 指摘 | 指摘のみ修正 + commit + push to same PR branch | `pull_request:synchronize` 発火、reviewer 自動再起動 (最大 3 回) |
| **automerge job** | — | 両 APPROVED + `pause-automation` / `needs-human-intervention` 不在 | `gh pr merge --squash --delete-branch` (head が claude-impl/* の時のみ delete-branch) | develop merged + Discord 通知 |
| **人間 (main merge 担当)** | — | develop 蓄積 → release PR (develop → main) | `gh pr merge --squash` (ADR-050 で `--delete-branch` 禁止) | 本番反映 |
