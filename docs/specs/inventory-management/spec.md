...
- **v1.3** (2026-05-26): **Phase A 長期方針を撤回、Phase B 即時移行に方針転換**（ひとしさん発、主導判断）。「今のスプレッドシートのデータをテストデータとして使用、CRM 完全移行時に必要分削除、今は CRM の在庫管理機能を最優先で完成させたい」方針。F9 / UF3 / Sprint 9 を Phase B 即時移行整備に再構成、Phase B を Out-of-scope #8 から in-scope に昇格。Issue #560（products INNER JOIN）は Phase A 制約 WONTFIX → **Phase B 移行で解消** に変更。Phase C（スプレッドシート完全閉鎖 + 過去データ完全移行）は引き続き Out-of-scope（時期未定、別 ADR）。
- **v1.2** (2026-05-22): **Phase A 並走長期化方針確定**（ひとしさん × しんごさん協議）。spec の F9 / UF3 / Sprint 9 を「段階的廃止 Phase A → B → C 切替」から「Phase A 並走運用整備」に絞り、Phase B / C（書込切替・スプレッドシート閉鎖）は Out-of-scope へ移動。Sprint 1-8 develop merged 後（PR #507-522）、Sprint 9 はこの絞り込み版で着手。**v1.3 で撤回**。
...
本スプリント群は、この **Discord 受信 → 正規化解析（ルール＋LLM ハイブリッド）→ 担当者承認 → 在庫反映 → 営業（在庫検索・見積・発注書）連携** の一気通貫を CRM 側に実装する。**v1.3 で方針転換**: スプレッドシートとの並走（Phase A）は撤回し、**現スプレッドシートのデータをテストデータとして CRM に投入、Phase B 即時移行で CRM 側を正本化**する。CRM 完全移行（Phase C 相当）時に不要なテストデータは必要分だけ削除する流れ。新規実装ではなく、既存資産（`backend/migrations/005|007|038`、`discord_gateway/` M2 段階、Meta webhook HMAC 検証パターン、`ProductsPage/QuotesPage/QuoteCreatePage/PurchaseOrdersPage/SuppliersPage` などの React ページ、Phase 1 の `require_permission()` パターン、ADR-027 i18n、ADR-034 で整備中の新規テナント migration 自動適用）をすべて拡張ベースで活用する。
...
- **G5**: **Phase B 即時移行で CRM 側を正本化**し、現スプレッドシートデータをテストデータとして投入完了させる（v1.3 確定）。Phase C（スプレッドシート完全閉鎖 + 過去データの本格データ移行）は CRM 軌道に乗ってから別 ADR で判断。
...
- **Acceptance criteria**:
  ...
  - **AC2.9 (v1.3 追加)**: TCG タブで「Pokemon Booster Box」を選択 → 配下の商品一覧画面 (`/super-admin/masters/tcg/{series_code}/products`) に遷移、その作品に属する `public.products` のみ表示・編集できる。新規追加・削除も可能 (`is_super_admin` 限定)。Pokemon BB / One Piece / Dragon Ball / Yu-Gi-Oh! / GUNDUM / Weiss Shwarz / Union Arena / Degimon / hololive / LORCANA / Xross Stars 各作品で同様。
  - **AC2.10 (v1.3 追加)**: TCG タブで「作品 (シリーズ) 自体の追加・削除」が可能 (`public.tcg_series_master` の CRUD)。例: 新作品「Lorcana」を追加 → 即座に Pokemon BB と並ぶ作品として選択可能になる。
...
- **モデル選定根拠**: Gemini 2.5 Flash は (i) 日本語精度が在庫メッセージの構造化抽出に十分、(ii) コスト効率が Claude Sonnet / Opus より大幅優位、(iii) 既存 Sales Anchor の他コンポーネントで運用実績あり、の 3 点。別モデル採用は別 ADR で正当化（memory: `project_jarvis_llm_gemini.md`）。
- **Acceptance criteria**:
...
- **設計注記（2026-05-26 v1.3 Phase B 即時移行で更新）**:
  - 検索結果の **candidate は `public.products` の行が存在することを前提**とする。`public.pokemon_dex` / `public.trainer_dex` / `public.tcg_series_master` / `public.supplier_aliases` のいずれかに hit しても、対応する `public.products` 行が無ければ candidate には含めない（**products INNER JOIN 前提**）。
  - 上記は AC7.4 が「候補から選択 → quote_items に標準名 + 標準 unit_price で行追加」と要求していることに整合する（products に紐づかない candidate は quote_items に乗せられない）。
  - **v1.3 Phase B 移行で AC7.1 / 7.3 / 7.4 / 7.5 を実機検証有効化**: Sprint 9 (revised) で `public.products` にテストデータ投入 (現スプレッドシート由来) 後、Playwright で実機検証する。
  - 実装側 (`backend/app/services/inventory_search.py`) の CTE `m_pokemon` / `m_trainer` / `m_tcg` / `m_alias` で `JOIN public.products` (INNER) を使っているのは本仕様に整合。**Issue #560 はバグでなく仕様、v1.2 で WONTFIX 扱いだったが v1.3 Phase B 移行で実害消失するため close**。
...
### F11: 仕入元現在オファー (inventory) エンティティ（v1.3 新規）

- **Background**: v1.3 でテストデータとして取り込む現スプレッドシートの「出力」シート (102 行) は、「仕入元 × 商品 × 状態 (Sealed/Damaged/No Shrink × Box/Case) × Quantity × Unit Price」の構造を持ち、これは spec v1.1〜v1.2 では未定義のエンティティ。営業フローで「仕入先 X が今 Y 個 Z 円で出してくれる」を直接参照する必要がある。
- **Description**: 新規 `public.inventory` テーブルを追加し、仕入元の現在オファーを保持する。F6 承認時に `inventory_movements` (履歴) と同時に `inventory` (現在状態) を更新する。
- **既存資産との関係**:
  - `public.products`: 商品マスタ (静的属性、Box重量・Release Date・Boxes per Case 等)
  - `public.inventory_movements`: 在庫増減履歴 (append-only)
  - `public.inventory` (F11 新規): 仕入元別現在オファー (UPSERT)
- **新規 migration**: `migrations/081_create_inventory.sql`
  - 列: `id, supplier_id (FK), product_id (FK), condition, quantity, unit_price (JPY), status, notes_ja, notes_en, offered_at, expires_at, source, created_at, updated_at`
  - UNIQUE 制約: `(supplier_id, product_id, condition)` で 1 行に集約
- **新規 ファイル**:
  - `backend/app/routers/inventory_offers.py` (新規、CRUD + 検索 API)
  - `backend/app/schemas/inventory_offers.py`
  - `frontend/src/pages/InventoryOffersPage.tsx` (admin / 営業向け、仕入元別オファー一覧)
- **Acceptance criteria**:
  - **AC11.1**: migration 081 で `public.inventory` が作成される (実 PostgreSQL で確認)
  - **AC11.2**: 出力.csv (102 行) を seed → `public.inventory` に 102 行投入される (supplier_id / product_id が正しく resolve、Mark 一致で product, 提供者一致で supplier)
  - **AC11.3**: F6 承認時に `inventory` が UPSERT される (`(supplier_id, product_id, condition)` キー)
  - **AC11.4**: F7 在庫検索結果に `inventory` の `quantity` と `unit_price` が表示される (products と JOIN、状態別表示)
  - **AC11.5**: 仕入元別オファー一覧 UI で admin が手動編集できる
  - **AC11.6**: Playwright で AC11.2 〜 AC11.5 を実機検証

### F9: Phase B 即時移行整備（v1.3 で書き直し）

- **Description**: **Phase B（CRM 正本化）に即時移行**する（v1.3 確定）。現スプレッドシートのデータを `sheets/*.csv` 経由でテストデータとして `public.products` / `public.knowledge_rules` / `public.supplier_aliases` 等に投入し、F6 承認操作で `products.stock_quantity` を実際に更新するパスを有効化する。スプレッドシートとの並走（Phase A）は廃止。**Phase C（スプレッドシート完全閉鎖 + 本格データ移行）は引き続き Out-of-scope**（時期未定、別 ADR）。
- **既存資産参照**:
  ...
  - 既存 seed scripts: `scripts/seed_pokemon_dex.py` / `seed_suppliers_from_sheet.py` / `seed_tcg_series.py`
- **Phase 定義**:
  - **Phase A（撤回済）**: ~~スプレッドシート並走~~ → v1.3 で撤回。
  - **Phase B（書込切替、v1.3 で in-scope）**: CRM 側 `products` を正本化、`products.stock_quantity` を F6 承認時に実更新。現スプレッドシートのデータは **テストデータとしてそのまま投入**、CRM 完全移行時に必要分だけ削除する流れ。
  - **Phase C（完全閉鎖、Out-of-scope）**: スプレッドシート閉鎖 + 過去データ完全移行（量・整合・ダウンタイム計画含む）。本仕様外、CRM 軌道に乗ってから別 ADR。
- **新規 / 更新ファイル**:
  - `migrations/070_add_spreadsheet_phase.sql`（既存）: `spreadsheet_phase` 列のデフォルトは Phase B に変更（既存 'A' レコードは Phase B に migrate）
  - `frontend/src/pages/admin/PhaseSwitchPage.tsx`（既存）: Phase B 表示に変更、Phase C ボタンのみ disabled（「別 ADR で検討中」ツールチップ）
  - `backend/app/services/phase_gate.py`（更新）: Phase A logic 撤去、Phase B 時に `inventory_movements` 記録 + `products.stock_quantity` 実更新を 1 トランザクションで行う
  - `scripts/seed_products_from_sheet.py`（新規）: `sheets/products.csv` から `public.products` に testdata 投入（既存 seed パターン踏襲、`product_code` UNIQUE で idempotent）
  - `scripts/seed_knowledge_rules_from_sheet.py`（新規）: `sheets/knowledge_rules.csv` から `public.knowledge_rules` 投入
  - `scripts/seed_supplier_aliases_from_sheet.py`（新規）: `sheets/supplier_aliases.csv` から `public.supplier_aliases` 投入
  - `scripts/export_inventory_for_sheet.py`（既存維持）: 一方向 CSV 出力ツールは残す（運用担当者が必要に応じてスプレッドシート側を更新するために使用、Phase B 中も並走で出力可能）
- **Acceptance criteria**:
  - **AC9.1**: Phase B 設定下で F6 の承認操作を実行 → `inventory_movements` に記録され、かつ **`products.stock_quantity` が承認通り更新**される（warning toast は表示されない、または「Phase B: CRM 正本化」のスナックバー表示）。
  - **AC9.2**: テストデータ投入後、`public.products` が **N 件以上**（N = スプレッドシート総商品数）、`public.knowledge_rules` / `public.supplier_aliases` も同様に空でないことを実機 PostgreSQL で確認（SC4 と整合）。
  - **AC9.3**: Phase 切替は admin のみ可能、`require_permission("phase.switch")`。`'B'` がデフォルト固定、`'C'` への切替は UI 上 disabled（「別 ADR で検討中」ツールチップ）。
  - **AC9.4**: Phase 切替履歴が `audit_log`（既存 or 新規）に記録される（将来 C 切替時の証跡用）。
  - **AC9.5**: Playwright で `/admin/phase-switch` を開き、現在 Phase B が表示され、C ボタンが disabled で「別 ADR で検討中」のツールチップが出ることを確認。
  - **AC9.6**: Sprint 6 の承認 UI で「Phase A: GS が真値」warning は **表示されない**（Phase B では正本化されているため）。
  - **AC9.7**: Sprint 7 の検索 UI で F7 AC7.1/7.3/7.4/7.5 を Playwright で実機検証 → テストデータ投入後の `public.products` 行に対して検索 hit + 見積行追加 + AND/OR が green。
...
### UF3: Phase B 即時移行フロー（admin、v1.3 で書き直し）

1. admin が Sprint 9 (revised) の seed scripts を VPS で実行 → `public.products` / `knowledge_rules` / `supplier_aliases` にテストデータ投入完了
2. `/admin/phase-switch` で **現在 Phase B** が表示される（migration 070 で default 'B' 化）
3. F5 (Discord 受信、もしくは admin 手動 inbound でも可) で `discord_inbound_messages` に新規行
4. 中央 admin がレビュー UI で承認（F6）→ **`inventory_movements` + `products.stock_quantity` を同一トランザクションで更新**
5. 営業が F7 在庫検索 → 投入済テストデータ + 承認反映の在庫数で検索 hit
6. F8 PO 発行 → PDF + alias 置換メール送信
7. Phase C 切替（スプレッドシート完全閉鎖 + 本格移行）は時期未定、別 ADR
...
- **SC4**: Phase B 移行後の E2E ハッピーパス (UF1 / UF3 v1.3) を Playwright で 1 回完走、テストデータ投入済 `public.products` の数が 0 以上、`products.stock_quantity` が承認通り更新されることを実機 PostgreSQL で確認。
- **SC5**: 営業 5 名がスプレッドシートを開かずに 1 週間 quote 作業を完結できる（運用観点、Phase B 移行直後から）。
...
### Sprint 10 (v1.3 新規): 作品ごとマスタ管理 UI + 作品グループ CRUD（F2 拡張）

- **Includes**: F2 AC2.9 / AC2.10
- **Background**: ひとしさん要求 (2026-05-26):
  「Pokemon BB / One Piece など作品名が入ったシートは、それぞれの商品の情報が含まれているマスタになります。これらのマスタも画面上で CRM の管理者だけがメンテナンスできるような画面も欲しいです。商品グループ（作品群）自体についても増減はあります。」
- **Definition of done**:
  - `/super-admin/masters/tcg/{series_code}/products` 階層ルート追加 (Pokemon BB / One Piece / Dragon Ball / Yu-Gi-Oh! / GUNDUM / Weiss Shwarz / Union Arena / Degimon / hololive / LORCANA / Xross Stars 各 11 作品で動作確認)
  - 作品ごとの `public.products` 一覧表示 + 検索 + 編集 + 追加 + 削除 (`is_super_admin` 限定)
  - TCG タブで作品 (シリーズ) 自体の CRUD: 新規作品追加、既存作品名/コード編集、削除 (FK 制約で products 残行のある作品は削除前 confirm)
  - i18n 同期、Playwright で AC2.9 / AC2.10 実機検証
- **Acceptance criteria covered**: AC2.9, AC2.10
- **Verification gates**: Cross-feature smoke #06 (Staff & Permissions)、Static analysis green、Fresh tenant onboarding
- **Out-of-scope (Sprint 10)**: 作品ごとの統計ダッシュボード、作品間商品移動、作品アーカイブ機能 (別 Sprint 候補)

### Sprint 11 (v1.3 新規): 仕入元現在オファー (inventory) エンティティ（F11）

- **Includes**: F11 AC11.1〜11.6
- **Definition of done**:
  - migration 081 作成 + deploy.yml 追記
  - `public.inventory` テーブルが本番反映、UNIQUE (supplier_id, product_id, condition) 制約有効
  - `seed_inventory_from_output_csv.py` で出力.csv 102 行を投入 (supplier 名・Mark で resolve)
  - F6 承認時の `inventory` UPSERT 実装 (`inventory_movements` と同一トランザクション)
  - F7 検索 UI に inventory.quantity / unit_price 表示
  - `/admin/inventory-offers` ページで仕入元別オファー CRUD (admin 限定)
  - Playwright で AC11.2〜11.5 実機検証 green
- **Acceptance criteria covered**: AC11.1〜11.6
- **Verification gates**: Cross-feature smoke #02 (Inventory Search)、Static analysis、Fresh tenant onboarding

### Sprint 9 (revised): Phase B 即時移行整備（F9、v1.3 で書き直し）

- **Includes**: F9（Phase B、テストデータ投入、F7 AC 再有効化）
- **Definition of done**:
  - `sheets/products.csv` / `knowledge_rules.csv` / `supplier_aliases.csv` 配置（Claude in Chrome 経由で現スプレッドシートから export）
  - 新規 seed scripts (`seed_products_from_sheet.py` / `seed_knowledge_rules_from_sheet.py` / `seed_supplier_aliases_from_sheet.py`) を VPS で実行、`public` 配下に投入
  - migration 070 を更新 (default 'A' → 'B'、既存レコードを Phase B に migrate するヘルパー migration 080 想定) → deploy.yml に追記
  - `/admin/phase-switch` で現在 Phase B が表示、Phase C のみ disabled
  - F6 承認時に `inventory_movements` 記録 + `products.stock_quantity` 実更新を同一トランザクションで実行（AC9.1）
  - F7 検索で AC7.1/7.3/7.4/7.5 を Playwright で実機検証 green（AC9.7）
  - Phase 切替履歴の audit_log 記録経路（C 切替時の証跡用）
- **Acceptance criteria covered**: AC9.1〜9.7
- **Verification gates**: Cross-feature smoke 全 8、Fresh tenant onboarding、SC4 / SC5 確認、Issue #560 close
- **Out-of-scope (Sprint 9 v1.3)**: Phase C スプレッドシート完全閉鎖、`scripts/import_inventory_from_sheet.py` のバルク本格移行（量・整合・ダウンタイム計画含む、すべて別 ADR で時期判断）
...
8. ~~Phase B 書込切替~~ → **v1.3 で in-scope に昇格 (Sprint 9 revised に組み込み)**
9. **Phase C スプレッドシート完全閉鎖** (v1.2 追加 / v1.3 維持): 本格的な過去データ完全移行 (量・整合性・ダウンタイム計画含む)、`scripts/import_inventory_from_sheet.py` のバルク import 本番運用。Phase B が安定運用 1-2 ヶ月後の別 ADR で判断。v1.3 のテストデータ投入とは別概念。
10. **既存 `tenant_004.products` → `public.products` データ移行** (v1.2 で範囲明確化 / v1.3 部分修正): v1.3 の Phase B 移行ではテストデータ投入のみ。本格データ移行 (整合性確認 / ダウンタイム計画含む) は Phase C 着手時、別 ADR でランブック化（Out-of-scope #5 と統合）。
...
- [x] **v1.3 追加**: Phase A 撤回、Phase B 即時移行に方針転換 (Overview / G5 / F9 / UF3 / Sprint 9 / SC4 / SC5 全更新)
- [x] **v1.3 追加**: 現スプレッドシートデータをテストデータとして投入する流れを Sprint 9 (revised) の DoD に明記
- [x] **v1.3 追加**: Issue #560 (products INNER JOIN) を Phase B 移行で close 扱い (WONTFIX 撤回)
- [x] **v1.3 追加**: F7 AC7.1/7.3/7.4/7.5 を Phase A 中 SKIP → Phase B 移行後 Playwright 実機検証 (AC9.7) に再有効化
- [x] **v1.3 追加**: F2 AC2.9 / AC2.10 + Sprint 10 で作品ごとの商品マスタ管理 UI と作品グループ CRUD を新規記載 (ひとしさん要求)
- [x] **v1.3 追加**: F11 + Sprint 11 で「仕入元現在オファー (`public.inventory`) エンティティ」を新規記載。出力.csv (仕入元×商品×状態×Quantity×Unit Price) の取り込み先として spec 化
...
**v1.2 更新（2026-05-22）**: Sprint 1-8 の develop merge 完了（PR #507-522）を踏まえ、ひとしさん × しんごさん協議で **Phase A 並走を当面長期運用形態として確定**。spec の F9 / UF3 / Sprint 9 を「段階的廃止 Phase A → B → C 切替」から「Phase A 並走運用整備」に絞り込み、Phase B（書込切替）/ Phase C（スプレッドシート閉鎖）は Out-of-scope #8/#9 に移動（時期未定、CRM 軌道に乗ってから別 ADR で判断）。これにより Sprint 9 は現実的な範囲で着手可能。Overview / Goals G5 も同方針に整合。memory: `project_jarvis_phase_a_long_term`。**v1.3 で撤回**。

**v1.3 更新（2026-05-26）**: ひとしさん主導判断で **Phase A 長期方針を撤回、Phase B 即時移行に方針転換**。「今のスプレッドシートのデータをテストデータとして使用、CRM 完全移行時に必要分だけ削除、今は CRM の在庫管理機能を最優先で完成させたい」方針。Overview / G5 / F7 設計注記 / F9 / UF3 / SC4 / SC5 / Sprint 9 / Out-of-scope #8 / Verification self-check を全面更新。Phase B は Out-of-scope #8 から in-scope (Sprint 9 revised) に昇格、Phase C は引き続き Out-of-scope #9 (本格データ移行は別 ADR、v1.3 のテストデータ投入とは別概念)。Issue #560 (products INNER JOIN WONTFIX) は Phase B 移行で実害消失するため close 扱い。F7 AC7.1/7.3/7.4/7.5 は AC9.7 で Playwright 実機検証 (テストデータ投入後)。新規 seed scripts (`seed_products_from_sheet.py` / `seed_knowledge_rules_from_sheet.py` / `seed_supplier_aliases_from_sheet.py`) を Sprint 9 (revised) で導入。memory: `project_jarvis_inventory_phase_b_pivot_2026_05_26`.
