...
- **設計注記（2026-05-25 Issue #560 結論を反映）**:
  ...
  - **Phase A 期間中の制約**: `public.products` は Discord 受信前のため 0 件状態が継続する（[[project-jarvis-phase-a-long-term]] 並走方針）。よって AC7.1 / 7.3 / 7.4 / 7.5 は **Phase A 中は実機検証 SKIP**、Phase B 移行（CRM 正本化で products 蓄積開始）後に follow-up Issue で検証する。
  - 実装側 (`backend/app/services/inventory_search.py`) の CTE `m_pokemon` / `m_trainer` / `m_tcg` / `m_alias` で `JOIN public.products` (INNER) を使っているのは本仕様に整合（Issue #560 はバグでなく Phase A 制約として WONTFIX）。
