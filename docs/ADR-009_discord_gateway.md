...
| 関連設計書 | `jarvis_crm_system_overview.docx` 第8章 8-2、`jarvis_crm_staff_roles_bots_design.docx` 第4章、`migrate_staff_roles_bots.md` 第3-5節 |
...
Jarvis CRM はマルチテナント B2B SaaS として、海外顧客との DM ベース営業を扱う。インバウンドメッセージング基盤は **Phase 2 で Meta Messenger（Facebook Messenger / Instagram DM）が稼働済み**で、`raw_webhook_events` → `meta_messages` → `leads` 自動登録のフローが確立している（`backend/app/routers/webhook.py`）。
...
- **G4**: 1 GB メモリ VPS の制約下で、安定稼働できる軽量プロセス構成にすること。
...
- **Tickets Bot / John-Bot**: 既存の役割（チケット管理・FAQ 等）を継続。Jarvis CRM とは独立。
...
4. **VPS 1 GB メモリ制約と整合**：Gateway worker は単一の WebSocket + asyncio タスクで動作し、120-180 MB 程度に収まる想定（discord.py 公式ベンチマーク + 1 ギルド規模）。
...
**現状（テナント 1 件 = HIGH LIFE JPN）**: HIGH LIFE JPN 専用の Bot を 1 つ作成し、そのトークンを `bots.api_key_hash` ではなく**新規に Bitwarden / GCP Secret Manager** で管理する。テーブル `bots` の `api_key_hash` は**Jarvis CRM 側 API への認証用**であり、**Discord Gateway トークンは別系統で保存**する（混同しない）。
...
- VPS のメモリ余剰確認（現状 1 GB 中、コアサービス稼働中の余剰量を sshで `free -m` 計測必須。Gateway worker 200 MB 確保できない場合は VPS 増強を別途検討）。
...
| R5 | VPS メモリ不足（1 GB 制約） | 高 | 中 | M2 着手前に `free -m` 計測。200 MB 確保できない場合は VPS 2GB プラン昇格を別途検討 |
...
| Q | 内容 | 回答が必要な理由 |
|---|------|-----------------|
| **Q1** | per-tenant Bot 案（5-3）を採用してよいか？仕様書原文「マルチテナントでの1 Bot運用」との整合は？ | アーキテクチャの根幹。現状テナント 1 件のため実害は小さいが、将来テナント追加時に決定が必要 |
| **Q2** | Discord 開発者アカウントは誰が管理するか？（しんごさん個人 / HIGH LIFE JPN 法人 / 開発パートナー） | M1 着手のブロッカー |
| **Q3** | Tickets Bot / John-Bot の運用責任者と機能範囲を再確認したい | R4 / R9 の前提情報 |
| **Q4** | VPS のメモリ余剰量（200 MB 確保可能か） | R5 / M2 着手判断 |
| **Q5** | Discord 連携の優先度（Phase 3 前半 / 後半 / Phase 4 以降） | 着手タイミング |
| **Q6** | Bot Token のローテーション頻度ポリシー（B-11 既存ポリシーに準拠でよいか） | 運用設計 |
| **Q7** | 営業 Bot が DM で実行できるアクションの範囲（送信のみか、ステータス更新も含めるか） | M4 のスコープ確定 |

### 10-2. 仕様書の追加更新提案

- `jarvis_crm_system_overview.docx` 第8章 8-2 の ADR-009 段落を、本 ADR の Accept 後に「ADR-009 採用済み（案 C ハイブリッド）」と1行更新すること。
- `jarvis_crm_staff_roles_bots_design.docx` 第4章末尾に「Discord トークンは `bots.api_key_hash` とは別系統で管理（ADR-009 5-5 参照）」を追記すること（現状記述が紛らわしい）。

### 10-3. 実装着手前の前提条件（チェックリスト）

- [ ] 本 ADR のステータスが Accepted に遷移
- [ ] Q1〜Q7 のすべてに回答
- [ ] Discord Bot Token が取得され Bitwarden に格納
- [ ] VPS メモリ余剰 200 MB 確保確認
- [ ] テスト Discord ギルド（HIGH LIFE JPN とは別）の準備
