...
**現状（テナント 1 件 = HIGH LIFE JPN）**: HIGH LIFE JPN 専用の Bot を 1 つ作成し、そのトークンを `bots.api_key_hash` ではなく**GitHub Secrets** で管理する。テーブル `bots` の `api_key_hash` は**Sales Anchor 側 API への認証用**であり、**Discord Gateway トークンは別系統で保存**する（混同しない）。
...
└── GitHub Secrets で保管 → 環境変数 DISCORD_BOT_TOKEN_<TENANT_ID> として注入
...
| トークンローテーション | Discord 側で Reset Token → GitHub Secrets 更新 → コンテナ再起動の手順をrunbook化（B-11 credential management policy 参照） |
...
|----|---------------|---------|---------|---------|
| **M1** | ADR レビュー + 設計確定 | 本 ADR のレビュー、しんごさん確認事項クリア、Discord 開発者アカウント・Bot Token 取得 | **0.5 日**（設計議論）+ 別途インフラ準備（しんごさん側 1〜2 日） | ADR ステータス Accepted、Bot Token が GitHub Secrets に格納 |
...
- **しんごさん側で M1 完了**（Bot Token 取得、GitHub Secrets 格納、開発者アカウント整備）が他マイルストーンの前提。
...
| R3 | Bot トークン漏洩 | 致命 | 低 | 環境変数管理 + GitHub Secrets + コンテナ分離 + 監査ログ redaction。Reset Token 手順を runbook 化 |
...
- [ ] Discord Bot Token が取得され GitHub Secrets に格納（**しんごさん側 M1 タスク**）
