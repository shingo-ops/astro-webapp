...
estimated_scale (Small/Large) → Discord ロール (Member/Partner) の自動付与。
...
ロールマッピング（固定）:
  Small  → Member  (Bot が未存在時に自動作成)
  Large  → Partner (Bot が未存在時に自動作成)
  Medium → マッピングなし（スキップ）
"""
...
# estimated_scale → Discord role name (固定マッピング)
_SCALE_TO_ROLE: dict[str, str] = {
    "Small": "Member",
    "Large": "Partner",
}

_MANAGED_ROLE_NAMES: frozenset[str] = frozenset(_SCALE_TO_ROLE.values())
...
async def _get_guild_id(tenant_id: int) -> str | None:
    """public.tenant_discord_config から guild_id を取得。"""
...
async def sync_lead_discord_role(
    *,
    ...
    role_name = _SCALE_TO_ROLE.get(new_scale)
    if not role_name:
    ...
    guild_id = await _get_guild_id(tenant_id)
    if not guild_id:
        # guild 未設定: 設定前は silent skip
        logger.info(
    ...
    try:
        ...
        for r in all_roles:
            if r.get("name") in _MANAGED_ROLE_NAMES and r.get("name") != role_name:
