...
| 担当 | Codex | Claude Code (Hikky-dev) |
|------|-------|------------------------|
| リサーチ | ✅ メイン（ライブラリ調査・GitHub検索・実現可否） | 補完のみ |
| プランニング | ✅ メイン（実装計画・タスク分解・技術選定案） | 補完のみ |
| ジェネレーター | ✅ メイン（コード実装・PR起票） | 自動フォールバック（Codex 不在時） |
| ジェネレーター | ✅ メイン（コード実装・PR起票） | 緊急対応・補完 |
| レビュー | — | ✅ メイン（コードレビュー・PR審査） |

- 新機能・バグ修正は Codex にリサーチ → プランニング → 実装を一貫委任するのが基本
- Claude Code は Codex の成果物（PR・設計案）をレビューする立場
- Codex が起案したプランは、ADR 正式化前に Claude Code がチェックする
