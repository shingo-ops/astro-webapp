...
## Codex を Generator として使う場合

```bash
# 対話モード（推奨 — 各ステップを確認しながら実装）
bash scripts/codex-generator.sh

# 自動モード（信頼済みコマンドを自動承認）
bash scripts/codex-generator.sh --auto
```

スプリントスペック (`.claude-pipeline/spec.md`) を自動読込して実装 → PR 作成まで行う。
PR 作成後は Claude Code の `reviewer` エージェントで審査すること。
