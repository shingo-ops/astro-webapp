...
- **feature ブランチ作業は常に** `bash scripts/new-worktree.sh feature/morimoto/<topic>` で開始（`git checkout -b` 直接禁止。pre-push フックで強制）
...
- **develop → main も PR 経由**（直push禁止・Branch Protection で強制）
  ...
  - **マージ方法は必ず "Create a merge commit"**（Squash 禁止 — squash すると back-merge が永続発生する）
...
**ルート `CLAUDE.md` が 120 行、`frontend/CLAUDE.md` が 90 行、`backend/CLAUDE.md` が 70 行を超えたら `frontend/scripts/check-claude-size.js` がコミットをブロックする。（ADR-076）**
