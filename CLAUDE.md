...
> **背景**: サイドナビ「インボックス」 / ページ h1 「受信箱」のようなズレが発生した。
> 再発防止のため、以下を **絶対ルール** とする。

### ルール: ページ h1/h2 は必ず `nav.*` キーを使う

```tsx
// ❌ 禁止: ページ固有の xxx.title キーを h1/h2 に使う
<h2>{t("inbox.title")}</h2>    // → サイドナビと別キーで文言がズレる
<h2>{t("leads.title")}</h2>

// ✅ 正しい: サイドナビと同じ nav.* キーを使う
<h2>{t("nav.leadChat")}</h2>   // → サイドナビ・ページ見出しが自動同期
<h2>{t("nav.leads")}</h2>
```

### 仕組み

サイドバー（`Layout.tsx`）と各ページ h1/h2 が **同一キー `nav.*`** を参照する。
`ja.json` / `en.json` の `nav.*` 値を変えるだけでサイドナビ・ページ見出しが同時に更新される。

```
nav.leadChat = "受信箱"
          ↓ 同一キーを参照
  サイドバー: "受信箱"   ←→   ページ h1: "受信箱"  （ズレ不可能）
```

### 新規ページ追加時のチェックリスト

1. `frontend/src/locales/ja.json` の `"nav"` セクションにキーを追加
2. `frontend/src/locales/en.json` にも同キーを追加（両ファイルのキー数一致）
3. `Layout.tsx` サイドバーで `t("nav.xxx")` を使う
4. ページコンポーネントの h1/h2 でも `t("nav.xxx")` を使う（`xxx.title` は作らない）
...
> ⚠️ 以下のルールは ESLint / scripts / CI で **自動的に強制** されています。
> 違反するとコミットが拒否されます（Husky pre-commit）、PR がマージできません（GitHub Actions CI）。

### 禁止: TSX インラインスタイルへの hex 色ハードコード
...
### 禁止: App.css 等への hex 色直書き

```css
/* ❌ check-css-hardcoded-colors.js がエラー → コミット不可 */
.card { background: #ffffff; }

/* ✅ CSS 変数を使う（index.css に定義されたもの） */
.card { background: var(--bg-surface); }
```

### 必須: 新しい CSS 変数を追加する時は必ずダーク側も追加

```css
/* index.css */
:root { --new-color: #xxx; }             /* ← 追加したら */
:root.force-dark { --new-color: #yyy; }  /* ← 必ずこちらも追加 */
```

追加を忘れると `check-dark-parity.js` が CI でエラーになります。

### よく使う CSS 変数

| 用途 | 変数名 |
|------|--------|
| ページ背景 | `var(--bg-primary)` |
| カード背景 | `var(--bg-surface)` |
| テーブル背景 | `var(--bg-subtle)` |
| ホバー背景 | `var(--bg-hover)` |
| 主テキスト | `var(--text-primary)` |
| 補助テキスト | `var(--text-secondary)` |
| 薄テキスト | `var(--text-muted)` |
| ボーダー | `var(--border)` |
| アクセント（青） | `var(--accent)` |
| accent 上テキスト | `var(--on-accent)` |
| 警告背景 | `var(--warning-bg)` |
| 警告テキスト | `var(--warning-text)` |
| 危険背景 | `var(--danger-bg)` |
| 成功背景 | `var(--success-bg)` |
...
```bash
cd frontend
npm run lint              # TSX hex 色チェック
npm run check:css-colors  # CSS hex 色チェック
