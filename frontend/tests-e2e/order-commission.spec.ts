...
test.describe("ADR-021 Sprint 5: 担当者報酬計算 MVP — 一覧 + パネル", () => {
  test("受注一覧に「報酬合計」列と「報酬編集」ボタンが描画される", async ({
    ...
    await expect(
      page.getByRole("columnheader", { name: "報酬合計" }),
    ).toBeVisible();
    // 未登録なので "-"
  ...
  test("「報酬編集」を開いて 5 ロールの担当者 select が表示される", async ({
    ...
    await expect(page.getByRole("dialog", { name: /担当者報酬/ })).toBeVisible();
  ...
  test("担当者 assign → 再計算で「報酬合計」セルが更新される", async ({
    ...
    // モーダルを閉じて一覧の合計セルを確認
    await page
      .getByRole("dialog", { name: /担当者報酬/ })
      .getByRole("button", { name: "閉じる" })
      .click();
    await expect(page.getByRole("dialog", { name: /担当者報酬/ })).toHaveCount(0);
