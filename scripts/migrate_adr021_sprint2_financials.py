#!/usr/bin/env python3
"""ADR-021 Phase 2 / Sprint 2 / Migration 047 用マイグレーションランナー。

実施内容:
  全テナントスキーマに order_financials テーブル + index + RLS ポリシーを作成する。
...
async def main() -> None:
    ...
    try:
        logger.info("=== ADR-021 Sprint 2 Migration 047 (order_financials) 開始 ===")
        ...
        tmpl = (MIGRATIONS_DIR / "047_create_order_financials.sql").read_text("utf-8")
        for tid, tc in tenants:
            ...
            try:
                ...
                logger.info(
                    "✓ %s (tenant_code=%s) order_financials 作成完了",
                    schema,
                    tc,
                )
        ...
        logger.info("=== ADR-021 Sprint 2 Migration 047 完了 ===")
    finally:
