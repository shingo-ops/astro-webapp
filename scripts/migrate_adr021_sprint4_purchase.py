#!/usr/bin/env python3
"""ADR-021 Phase 4 / Sprint 4 / Migration 049 用マイグレーションランナー。

実施内容:
  全テナントスキーマに order_purchase_details テーブル + index + RLS ポリシーを作成する。
...
async def main() -> None:
    ...
    try:
        logger.info("=== ADR-021 Sprint 4 Migration 049 (order_purchase_details) 開始 ===")
        ...
        tmpl = (MIGRATIONS_DIR / "049_create_order_purchase_details.sql").read_text("utf-8")
        for tid, tc in tenants:
            ...
            try:
                ...
                logger.info(
                    "✓ %s (tenant_code=%s) order_purchase_details 作成完了",
                    schema,
                    tc,
                )
        ...
        logger.info("=== ADR-021 Sprint 4 Migration 049 完了 ===")
    finally:
