#!/usr/bin/env python3
"""ADR-021 J1 fix / Migration 051 用マイグレーションランナー。

実施内容:
  全テナントスキーマの `orders.status='confirmed'` 行を `'pending'` に統合する。

冪等:
  WHERE status='confirmed' で自然に保証。2 回目以降は 0 行更新。
...
注意:
  - order_purchase_details.purchase_status='confirmed'（仕入確定フラグ）は
    本 migration の対象外。別ドメインの値であり、絶対に触らない。
"""
...
async def main() -> None:
    ...
    try:
        logger.info("=== ADR-021 J1 fix / Migration 051 (remove confirmed) 開始 ===")
        ...
        tmpl = (MIGRATIONS_DIR / "051_remove_confirmed_status.sql").read_text("utf-8")
        for tid, tc in tenants:
            ...
            try:
                ...
                logger.info(
                    "✓ %s (tenant_code=%s) confirmed → pending 統合完了",
                    schema,
                    tc,
                )
        ...
        logger.info("=== ADR-021 J1 fix / Migration 051 完了 ===")
    finally:
