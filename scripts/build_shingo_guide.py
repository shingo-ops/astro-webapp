...
add_para(doc, "Jarvis CRM B-2: Discord 連携 M1 タスク（しんごさん作業分）",
...
add_para(doc,
         "Jarvis CRM の Discord 連携機能（リードからの DM 受信 + 自動返信）を本番稼働させる"
         "ため、しんごさんに 4 つのセットアップ作業をお願いします。所要時間は合計 30〜60 分程度です。",
         size=11)
add_callout(doc, "💡 たとえ話",
            "これは「お店に新人スタッフ（配達員）を採用する」作業によく似ています。Bot は Jarvis CRM 専用の "
...
add_para(doc,
         ...
         "アプリを登録する必要があります。Jarvis CRM はテナント（HIGH LIFE JPN）ごとに "
...
add_step(doc, 4, "アプリ名を入力。例: Jarvis CRM (HIGH LIFE JPN)")
...
add_step(doc, 5, "エントリ名: 例: Discord Bot Token - Jarvis CRM (HIGH LIFE JPN)")
...
add_step(doc, 3, "サーバー名: Jarvis CRM Test など分かりやすい名前で OK")
...
add_step(doc, 5, "コピーした URL をブラウザで開く → 招待先に「Jarvis CRM Test」サーバーを選択 → 認証")
...
add_para(doc,
         ...
         "M3（受信メッセージを Jarvis CRM に取り込む処理）は別セッションで実装します。",
         size=11)
