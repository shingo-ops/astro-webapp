#!/usr/bin/env python3
"""Phase 5 マイグレーション（シフト/Buddy/バッジ/ERP）。"""
...
async def main():
    ...
    try:
        logger.info("=== Phase 5 マイグレーション開始 ===")
        sql = (BASE_DIR / "migrations" / "010_add_phase5_permissions.sql").read_text("utf-8")
        async with engine.begin() as conn: await _exec(conn, sql)
        logger.info("✓ パーミッション追加完了")

        async with engine.connect() as conn:
            r = await conn.execute(text("SELECT id, tenant_code FROM public.tenants WHERE is_active = true ORDER BY id"))
            tenants = [(row.id, row.tenant_code) for row in r]
        logger.info("対象テナント: %d", len(tenants))

        tmpl = (BASE_DIR / "migrations" / "011_add_phase5_tenant_tables.sql").read_text("utf-8")
        for tid, tc in tenants:
            ...
            logger.info("✓ tenant_%03d テーブル追加", tid)
            async with engine.begin() as conn:
                await conn.execute(text(f"SET search_path = {schema}, public"))
                await conn.execute(text(f"SET app.tenant_id = '{tid}'"))
                await seed_system_roles(conn, tid, schema)
            logger.info("✓ tenant_%03d ロール再シード", tid)
        logger.info("=== Phase 5 マイグレーション完了 ===")
    finally:
