#!/usr/bin/env python3
"""Inventory Sprint 8 (migration 069) 用ランナー。

spec.md v1.1 Sprint 8 / F8 の migration バンドルを適用する。

順序:
  1. 069_create_tenant_profile.sql  (テナント別 + permissions seed)

migration 069 は {tenant_xxx} schema にテーブルを作り、同時に
public.permissions に tenant.profile.view / tenant.profile.edit を seed する
(1 ファイルで完結、テナントループ + 権限割当)。

実行方法 (VPS 側):
  docker compose exec -e TENANT_CODE=highlife-jpn -w /app backend \\
      python /app/scripts/migrate_inventory_sprint8.py

前提:
  - Sprint 1-7 (056〜068) が適用済み
  - DATABASE_URL が postgresql:// または postgresql+asyncpg:// 形式

冪等:
  - 069: CREATE TABLE IF NOT EXISTS / ADD COLUMN IF NOT EXISTS /
        ON CONFLICT (key) DO NOTHING / WHERE NOT EXISTS

関連:
  scripts/migrate_inventory_sprint1.py (パターン踏襲)
  scripts/migrate_inventory_sprint2.py (パターン踏襲)
  migrations/069_create_tenant_profile.sql
  .claude-pipeline/spec.md Sprint 8 / F8
"""
...
async def main() -> int:
    ...
    try:
        for filename in SPRINT8_MIGRATIONS:
            try:
    ...
    logger.info("Sprint 8 migrations all applied")
    return 0
