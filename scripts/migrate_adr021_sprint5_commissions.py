#!/usr/bin/env python3
"""ADR-021 Phase 5 / Sprint 5 / Migration 050 用マイグレーションランナー。

実施内容:
  全テナントスキーマに対して 3 つの DDL を一括適用する。
    a) staff.is_employee BOOLEAN カラム追加
    b) tenant_commission_settings テーブル + index + RLS ポリシー作成
    c) order_commissions テーブル + index + RLS ポリシー作成

冪等:
  ALTER TABLE IF NOT EXISTS / CREATE TABLE IF NOT EXISTS /
  CREATE INDEX IF NOT EXISTS / DO ブロックでポリシー存在確認。
  何度実行しても副作用は発生しない。
...
前提:
  - migration 005（orders テーブル本体）と migration 019（staff テーブル本体）が
    全テナントに適用済み
...
async def main() -> None:
    ...
    try:
        logger.info("=== ADR-021 Sprint 5 Migration 050 (commissions) 開始 ===")
        ...
        tmpl = (MIGRATIONS_DIR / "050_add_commissions.sql").read_text("utf-8")
        for tid, tc in tenants:
            ...
            try:
                ...
                logger.info(
                    "✓ %s (tenant_code=%s) commissions DDL 適用完了",
                    schema,
                    tc,
                )
        ...
        logger.info("=== ADR-021 Sprint 5 Migration 050 完了 ===")
    finally:
