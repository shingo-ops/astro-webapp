...
--   public.permissions に存在する権限を INSERT（ON CONFLICT DO NOTHING）。
--   tenant.py の _assign_permissions_to_role() 契約に合わせる:
--     オーナー        : ALL（system.manage を含む全権限）
--     システム管理者  : ALL_EXCEPT_SYSTEM_MANAGE（system.manage を除外）
--   既存の割当は保持、不足分のみ追加される。
--
-- 冪等性:
